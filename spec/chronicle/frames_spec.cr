...
module FramesSpecHelper
  ...
  def runtime : Chronicle::Runtime(FrameModel)
    store = Chronicle::MemoryEventStore.new
    agent = Crig::Agent(FrameModel).new(model: FrameModel.new)
    la = Chronicle::LogAgent(FrameModel).new(agent, store: store)
    Chronicle::Runtime(FrameModel).new(store: store, log_agent: la)
  end

  def event(id : String, type : String, frame_id : String?) : Chronicle::Event
    Chronicle::Event.new(
...
describe Chronicle::EventLogCodec do
  it "round-trips frame_id through encode/decode" do
    ...
    log = Chronicle::EventLog.from_events([event])
...
describe Chronicle::Runtime do
  it "pushes and pops frames and stamps events with the current frame" do
    ...
    runtime.push_frame(Chronicle::Frame.new(goal: "research", id: "frame_1"))
