...
# tests/test_pattern_matcher.py (revision 148e12c2969f18fa12a1a3c2e75f3affd9aa0616).
...
module PatternsMatcherSpecHelper
  ...
  def graph_with_two_claims_one_contradicts : Chronicle::GraphProjection
    seq = 1_u64
    ...
    [a, b, c, r1, r2].reduce(Chronicle::GraphProjection.empty) { |g, e| g.apply(e) }
  end
end

describe Chronicle::PatternMatcher do
  it "matches a single node by type" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(c:claim)").compile
  ...
  it "matches node property equality" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(c:claim {confidence: 0.9})").compile
    matches = matcher.matches(nil, g)
  ...
  it "matches a relationship by type" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(a:claim)-[:contradicts]->(b:claim)").compile
    matches = matcher.matches(nil, g)
    matches.size.should eq(1)
  ...
  it "binds a relationship variable" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(a:claim)-[r:contradicts]->(b:claim)").compile
    matches = matcher.matches(nil, g)
    matches.size.should eq(1)
  ...
  it "matches a directed left relationship" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(b:claim)<-[:contradicts]-(a:claim)").compile
    matches = matcher.matches(nil, g)
    matches.size.should eq(1)
  end
  ...
  it "returns no match when no relationship exists" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(a:claim)-[:cites]->(b:claim)").compile
  ...
  it "matches a multi-hop chain" do
    ...
    g = [a, b, c, r1, r2].reduce(Chronicle::GraphProjection.empty) { |graph, e| graph.apply(e) }

    matcher = Chronicle.parse("(a:claim)-[:cites]->(b:source)-[:in]->(c:doc)").compile
    matches = matcher.matches(nil, g)
    matches.size.should eq(1)
  ...
  it "filters matches with a WHERE comparison" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(c:claim) WHERE c.confidence > 0.5").compile
    matches = matcher.matches(nil, g)
  ...
  it "filters matches with WHERE AND" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(c:claim) WHERE c.confidence > 0.7 AND c.status = \"open\"").compile
    matches = matcher.matches(nil, g)
    matches.size.should eq(2)
  end
  ...
  it "inverts matches with WHERE NOT" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(c:claim) WHERE NOT c.confidence > 0.5").compile
    matches = matcher.matches(nil, g)
    matches.size.should eq(1)
  end
  ...
  it "excludes subpatterns matched by NOT EXISTS" do
    ...
    g = [a, b, x, r1].reduce(Chronicle::GraphProjection.empty) { |graph, e| graph.apply(e) }
  ...
  it "returns an empty list when no matches exist" do
    ...
    g = Chronicle::GraphProjection.empty.apply(ev)
    matcher = Chronicle.parse("(c:claim) WHERE c.confidence > 0.99").compile
    matcher.matches(nil, g).should be_empty
  end

  it "resolves a bare var path to the object id" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(c:claim) WHERE c = \"obj_a\"").compile
    matches = matcher.matches(nil, g)
    matches.size.should eq(1)
  ...
  it "resolves object field access via c.type" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(c) WHERE c.type = \"claim\"").compile
    matches = matcher.matches(nil, g)
    matches.size.should eq(3)
  end
  ...
  it "returns no match when ordered comparison operands are nil" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(c:claim) WHERE c.confidence > c.missing").compile
    matcher.matches(nil, g).should be_empty
  end

  it "raises on ordered comparison of incomparable types" do
    g = PatternsMatcherSpecHelper.graph_with_two_claims_one_contradicts
    matcher = Chronicle.parse("(c:claim) WHERE c.confidence > c.status").compile
    expect_raises(Chronicle::PatternTypeError) do
