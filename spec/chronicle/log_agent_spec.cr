...
describe Chronicle::LogAgent do
  it "constructs from a Crig::Agent" do
    crig_agent = Crig::Agent(MockModel).new(model: MockModel.new)
    log_agent = Chronicle::LogAgent(MockModel).new(crig_agent)
    log_agent.should be_a(Chronicle::LogAgent(MockModel))
  end
  ...
  it "builds an EffectRequest for a CallModel step" do
    ...
    effect.as(Chronicle::EffectRequest).kind.should eq(Chronicle::EffectKind::Model)
    effect.as(Chronicle::EffectRequest).payload.should contain("Hello")
  end
  ...
  it "produces tool EffectRequests after feeding a model response with tool calls" do
    ...
    choice = Crig::OneOrMany(Crig::Completion::AssistantContent).one(
      Crig::Completion::AssistantContent.tool_call("tc_001", "web_search", JSON.parse(%({"q":"chronicle"})))
    )
    ...
    effects.first.kind.should eq(Chronicle::EffectKind::Tool)
  ...
  it "completes a full model→tools→done cycle" do
    ...
    log_agent = Chronicle::LogAgent(MockModel).new(crig_agent, max_turns: 2)
    ...
    choice1 = Crig::OneOrMany(Crig::Completion::AssistantContent).one(
      Crig::Completion::AssistantContent.tool_call("tc_001", "search", JSON.parse(%({"q":"chronicle"})))
    )
