...
describe Chronicle::LLMCache do
  ...
  it "records and retrieves a response by hash" do
    cache = Chronicle::LLMCache.new
    result = Chronicle::EffectResult.new("abc123", true, %({"text":"hello"}))
  ...
  it "populates from a sequence of effect.requested + effect.responded events" do
    ...
    cache = Chronicle::LLMCache.from_events(events)
  ...
  it "serves cached responses during replay" do
    cache = Chronicle::LLMCache.new
    effect = Chronicle::EffectResult.new("abc123", true, %({"text":"cached"}))

    cache.record("abc123", effect)
    engine = Chronicle::ReplayEngine.new

    events = [] of Chronicle::Event
    replay_result = engine.replay(events, Chronicle::ReplayMode::Permissive, [] of Chronicle::Event, nil, cache)
