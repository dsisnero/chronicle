...
module PackSpecHelper
  ...
  def runtime(tools : Array(Chronicle::Tool) = [] of Chronicle::Tool) : Chronicle::Runtime(PackModel)
    ...
    Chronicle::Runtime(PackModel).new(store: store, log_agent: la, tools: tools)
  end
end

describe Chronicle::Pack do
  it "holds pack identity and contents" do
    search = Chronicle::Tool.new("search", "search") { |a| "{}" }
    pack = Chronicle::Pack.new(
  ...
  it "validates name and version" do
    expect_raises(Chronicle::PackError, /name/) do
      Chronicle::Pack.new(name: "Bad Name", version: "1.0.0")
    end
    expect_raises(Chronicle::PackError, /version/) do
      Chronicle::Pack.new(name: "ok", version: "")
    end
  end
end
...
describe Chronicle::Runtime do
  it "loads a pack, registers its tools, and records a pack.loaded event" do
    search = Chronicle::Tool.new("search", "search") { |a| "{}" }
    pack = Chronicle::Pack.new(name: "diligence", version: "1.0.0", tools: [search])
  ...
  it "surfaces pack policies and approval routing" do
    ...
    pack = Chronicle::Pack.new(name: "p", version: "1.0.0", policies: [policy])
    runtime = PackSpecHelper.runtime
  ...
  it "routes a policy-required tool call through pending approval instead of executing" do
    ...
    pack = Chronicle::Pack.new(name: "p", version: "1.0.0", tools: [search], policies: [policy])
    runtime = PackSpecHelper.runtime
