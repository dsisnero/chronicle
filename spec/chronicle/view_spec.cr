...
describe Chronicle::View do
  it "full view contains all objects" do
    ...
    view = graph.build_view(Chronicle::ViewSpec.new)
  ...
  it "filters objects by type" do
    ...
    spec = Chronicle::ViewSpec.new(include_types: ["task"])
  ...
  it "filters objects by type and relations are included" do
    ...
    spec = Chronicle::ViewSpec.new(include_types: ["claim"])
    view = graph.build_view(spec)
  ...
  it "limits recent events" do
    ...
    graph = events.reduce(Chronicle::GraphProjection.empty) { |g, e| g.apply(e) }

    spec = Chronicle::ViewSpec.new(recent_events: 3)
    view = graph.build_view(spec)
  ...
  it "filters objects anchored around a specific object" do
    ...
    spec = Chronicle::ViewSpec.new(around: "obj_001", include_types: ["task"])
    view = graph.build_view(spec)
    view.objects.size.should eq(1)
