...
private def channel_runtime
  ...
  log_agent = Chronicle::LogAgent(ChannelSpecModel).new(agent, store: store)
  {Chronicle::Runtime(ChannelSpecModel).new(store: store, log_agent: log_agent), store}
end

describe "Channel command ingress" do
  it "records command provenance and a causal chat chain" do
    runtime, store = channel_runtime
    command = Chronicle::Channel::SendMessage.new(
