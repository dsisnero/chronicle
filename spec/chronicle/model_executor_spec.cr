...
private class RecordingProviderFactory < Chronicle::CrigProviderFactory
  getter calls = [] of {String, String, String}

  def initialize(@executor : Chronicle::ModelExecutor)
  end

  def build(instance_id : String, config : Chronicle::ProviderConfig, target : Chronicle::Routing::Target) : Chronicle::ModelExecutor
    @calls << {instance_id, config.type, target.model}
    @executor
  end
end

describe Chronicle::FixedModelExecutor do
  it "executes the registered model for its target" do
    model = FixedExecutorSpecModel.new
    target = Chronicle::Routing::Target.new("deepseek", "deepseek-v4-flash")
    executor = Chronicle::FixedModelExecutor(FixedExecutorSpecModel).new(model)
...
describe Chronicle::ProviderRegistry do
  it "builds an exact target registry through the configured provider kind" do
    ...
    providers:
      ...
      "openai-compat:lab":
        ...
        local: true
    YAML
    ollama_target = Chronicle::Routing::Target.new("ollama", "qwen2.5-coder", false)
    compat_target = Chronicle::Routing::Target.new("openai-compat:lab", "llama-3.1", false)
    executor = Chronicle::FixedModelExecutor(FixedExecutorSpecModel).new(FixedExecutorSpecModel.new)
    ...
    registry = Chronicle::ProviderRegistry.from_config(
      ...
      {"ollama" => ollama_factory.as(Chronicle::CrigProviderFactory), "openai_compatible" => compat_factory.as(Chronicle::CrigProviderFactory)},
    )
  ...
  it "does not substitute a different factory for an unsupported provider kind" do
    ...
    providers:
      gemini:
        ...
        api_key: test
    YAML
    target = Chronicle::Routing::Target.new("gemini", "gemini-2.5-flash")
    executor = Chronicle::FixedModelExecutor(FixedExecutorSpecModel).new(FixedExecutorSpecModel.new)
    ...
    registry = Chronicle::ProviderRegistry.from_config(
      config,
      [target],
      {"ollama" => wrong_factory.as(Chronicle::CrigProviderFactory)},
    )
  ...
  it "constructs a keyless local Ollama executor without contacting the endpoint" do
    ...
    registry = Chronicle::ProviderRegistry.from_config(
      config,
      [target],
      {"ollama" => Chronicle::OllamaProviderFactory.new.as(Chronicle::CrigProviderFactory)},
    )
  ...
  it "constructs cloud Crig executors from configured credentials without network I/O" do
    ...
    YAML
    targets = [
      Chronicle::Routing::Target.new("deepseek", "deepseek-chat"),
      Chronicle::Routing::Target.new("openai", "gpt-4o"),
      Chronicle::Routing::Target.new("anthropic", "claude-sonnet"),
      Chronicle::Routing::Target.new("gemini", "gemini-2.5-flash"),
    ]

    registry = Chronicle::ProviderRegistry.from_config(config, targets, Chronicle::ProviderFactories.defaults)
  ...
  it "uses a named OpenAI-compatible instance rather than the built-in OpenAI instance" do
    ...
    providers:
      "openai-compat:lab":
        ...
        local: true
    YAML
    target = Chronicle::Routing::Target.new("openai-compat:lab", "llama-3.1-70b", false)

    registry = Chronicle::ProviderRegistry.from_config(config, [target], Chronicle::ProviderFactories.defaults)

    registry.registered?(target).should be_true
  end
end

describe Chronicle::ModelEffectWorker do
  it "executes a typed effect invocation and returns a normalized edge result" do
    model = FixedExecutorSpecModel.new
    target = Chronicle::Routing::Target.new("ollama", "qwen2.5-coder", false)
    effect = Chronicle::EffectRequest.new("effect-1", Chronicle::EffectKind::Model, %({"prompt":"hello"}))
    request = Chronicle::ModelEffectRequest.new("llm_requested_1", effect, target)
    invocation = Chronicle::ModelEffectInvocation.new(request, model.completion_request("hello").build)
    worker = Chronicle::ModelEffectWorker.new(
      Chronicle::ProviderRegistry.new.register(target, Chronicle::FixedModelExecutor(FixedExecutorSpecModel).new(model)),
    )
