...
describe Chronicle::TUI::StandaloneBubbleTeaModel do
  it "implements Tea::Model interface" do
    model = Chronicle::TUI::StandaloneBubbleTeaModel.new
  ...
  it "update handles enter key" do
    ...
    model.update(key)
    model.program.state.should eq(Chronicle::TUI::State::Processing)
  ...
  it "view returns a Tea::View with header" do
    ...
    view.content.should contain("Chronicle Agent")
  end
  ...
  it "quits and restores the terminal on Ctrl-C" do
    ...
    updated.should be(model)
    model.program.state.should eq(Chronicle::TUI::State::Done)
