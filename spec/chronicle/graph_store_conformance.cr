...
# activegraph/store/graph_conformance.py (revision 148e12c2969f18fa12a1a3c2e75f3affd9aa0616).
...
module GraphStoreConformanceFixture
  ...
  def obj(id : String, type : String = "memo", data : String = %({"text":"hello #{id}","n":1}), version : Int64 = 1) : Chronicle::GraphObject
    Chronicle::GraphObject.new(id: id, type: type, data: data, version: version)
  end

  def rel(id : String, source : String, target : String, type : String = "links") : Chronicle::GraphRelation
    Chronicle::GraphRelation.new(id: id, type: type, from_id: source, to_id: target)
  end

  def patch(id : String, target : String, status : Chronicle::PatchState = Chronicle::PatchState::Proposed) : Chronicle::Patch
    Chronicle::Patch.new(
      id: id, target: target, op: Chronicle::PatchOp::Update,
      value: %({"text":"new"}), expected_version: 1_i64,
      proposed_by: "test", status: status,
    )
  end

  def chain_ids(chains : Array(Chronicle::ChainMatch))
...
module GraphStoreConformance
  ...
  macro define_tests(store_factory)
    ...
    it "round-trips patches" do
      ...
      got.op.should eq(Chronicle::PatchOp::Update)
      got.status.should eq(Chronicle::PatchState::Proposed)
    end

    it "overwrites a patch status on put" do
      store = {{store_factory}}
      store.put_patch(GraphStoreConformanceFixture.patch("patch_1", "obj_1", status: Chronicle::PatchState::Proposed))
      store.put_patch(GraphStoreConformanceFixture.patch("patch_1", "obj_1", status: Chronicle::PatchState::Applied))
      got = store.get_patch("patch_1").not_nil!
      got.status.should eq(Chronicle::PatchState::Applied)
