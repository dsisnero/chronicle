...
describe Chronicle::ToolCache do
  it "stores and retrieves tool results by name+args hash" do
    cache = Chronicle::ToolCache.new
    cache.record("web_search", %({"q":"chronicle"}), %({"result":"found"}))
    cache.size.should eq(1)

    result = cache.get("web_search", %({"q":"chronicle"}))
  ...
  it "populates from tool.responded events" do
    req = Chronicle::Event.new(
      ...
      payload: %({"tool":"web_search","args":{"q":"chronicle"},"call_id":"call_001"}),
    )
    resp = Chronicle::Event.new(
      schema_version: 1_u16, sequence: 2_u64, id: "tool_resp_001",
      type: "tool.responded", actor: "agent", caused_by: "tool_req_001",
      timestamp: Time.utc(2026, 7, 25, 12, 0, 0),
      payload: %({"tool":"web_search","args":{"q":"chronicle"},"output":{"result":"found"},"call_id":"call_001"}),
    )

    cache = Chronicle::ToolCache.from_events([req, resp])
    cache.size.should eq(1)
  ...
  it "skips failed tool responses" do
    ...
    cache = Chronicle::ToolCache.from_events([resp])
