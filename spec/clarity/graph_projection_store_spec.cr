...
describe Clarity::GraphProjection do
  ...
  it "delegates reads to the injected store" do
    ...
    store.put_relation(Clarity::GraphRelation.new(id: "rel_1", type: "links", from_id: "task#1", to_id: "task#2"))
    g = Clarity::GraphProjection.new(store: store)
    g.get_object("task#1").not_nil!.type.should eq("task")
  ...
  it "replays deterministically regardless of backend instance" do
    ...
    a = Clarity::GraphProjection.replay(events)
    b = Clarity::GraphProjection.replay(events)
