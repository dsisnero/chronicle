...
# tools/cache.py (revision 8aedb1866cf5dce056af97529152ffd6f468a1ed).
...
module ToolSpecHelper
  ...
  def graph(events : Array(Clarity::Event)) : Clarity::GraphProjection
    events.reduce(Clarity::GraphProjection.empty) { |g, e| g.apply(e) }
  end
end
...
describe Clarity do
  describe "make_graph_query_tool" do
    it "returns object refs matching the type and where filter" do
      ...
      tool = Clarity.make_graph_query_tool(g)
  ...
  describe Clarity::ToolRegistry do
    it "registers, snapshots, and clears tools" do
      Clarity::ToolRegistry.clear
      tool = Clarity::Tool.new("echo", "returns args") { |args| args }
      Clarity::ToolRegistry.register(tool)
      Clarity::ToolRegistry.snapshot.map(&.name).should eq(["echo"])
      Clarity::ToolRegistry.clear
      Clarity::ToolRegistry.snapshot.should be_empty
    end
  end
end
...
describe Clarity::ToolCache do
  it "serves cached tool results during replay" do
    ...
    cache = Clarity::ToolCache.from_events([
      Clarity::Event.new(
...
describe Clarity::Runtime do
  it "invokes a registered tool and records tool.requested/responded" do
    store = Clarity::MemoryEventStore.new
    search = Clarity::Tool.new("search", "search for things") { |args| %({"results":[#{args}]}) }
    agent = Crig::Agent(ToolCallingModel).new(model: ToolCallingModel.new, preamble: "Use tools.")
    la = Clarity::LogAgent(ToolCallingModel).new(agent, store: store, max_turns: 2)
    runtime = Clarity::Runtime(ToolCallingModel).new(store: store, log_agent: la, tools: [search])
