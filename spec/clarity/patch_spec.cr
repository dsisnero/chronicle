...
describe Clarity::GraphProjection do
  it "proposes a patch on an object" do
    ...
    patch.status.should eq(Clarity::PatchState::Proposed)
  ...
  it "applies a proposed patch and increments version" do
    ...
    stored.not_nil!.status.should eq(Clarity::PatchState::Applied)
  end
  ...
  it "rejects a patch on version mismatch" do
    ...
    graph = graph.apply_patch(patch.id)
    stored = graph.get_patch(patch.id)
    stored.not_nil!.status.should eq(Clarity::PatchState::Rejected)
  ...
  it "raises on double-apply of same patch" do
    ...
    expect_raises(Clarity::GraphProjectionError, /already/) do
