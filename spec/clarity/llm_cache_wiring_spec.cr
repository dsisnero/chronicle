...
module LLMCacheWiringSpecHelper
  ...
  def event(id : String, type : String, caused_by : String?, payload : String) : Clarity::Event
    Clarity::Event.new(
...
describe Clarity::LLMCache do
  it "harvests llm.responded events keyed by the request hash" do
    ...
    cache = Clarity::LLMCache.from_events([requested, responded])
  ...
  it "skips error-shaped llm.responded events" do
    ...
    cache = Clarity::LLMCache.from_events([requested, failed])
...
describe Clarity::Runtime do
  it "serves a cached response without calling the provider" do
    store1 = Clarity::MemoryEventStore.new
    agent = Crig::Agent(CacheMockModel).new(model: CacheMockModel.new)
    la = Clarity::LogAgent(CacheMockModel).new(agent, store: store1)
    runtime = Clarity::Runtime(CacheMockModel).new(store: store1, log_agent: la)
    first = runtime.run("Cache me")

    cache = Clarity::LLMCache.from_events(store1.iter_events)
    cache.size.should be >= 1

    store2 = Clarity::MemoryEventStore.new
    agent2 = Crig::Agent(ExplodingModel).new(model: ExplodingModel.new)
    la2 = Clarity::LogAgent(ExplodingModel).new(agent2, store: store2)
    runtime2 = Clarity::Runtime(ExplodingModel).new(store: store2, log_agent: la2, llm_cache: cache)
  ...
  it "records a cache_hit flag on served llm.requested events" do
    store1 = Clarity::MemoryEventStore.new
    agent = Crig::Agent(CacheMockModel).new(model: CacheMockModel.new)
    runtime = Clarity::Runtime(CacheMockModel).new(store: store1, log_agent: Clarity::LogAgent(CacheMockModel).new(agent, store: store1))
    runtime.run("Flag me")

    cache = Clarity::LLMCache.from_events(store1.iter_events)
    store2 = Clarity::MemoryEventStore.new
    agent2 = Crig::Agent(ExplodingModel).new(model: ExplodingModel.new)
    runtime2 = Clarity::Runtime(ExplodingModel).new(store: store2, log_agent: Clarity::LogAgent(ExplodingModel).new(agent2, store: store2), llm_cache: cache)
  ...
  it "populates the cache on Runtime.load with replay_llm_cache" do
    ...
    agent = Crig::Agent(CacheMockModel).new(model: CacheMockModel.new)
    runtime = Clarity::Runtime(CacheMockModel).new(store: store, log_agent: Clarity::LogAgent(CacheMockModel).new(agent, store: store))
    runtime.run("Load cache")

    agent2 = Crig::Agent(ExplodingModel).new(model: ExplodingModel.new)
    loaded = Clarity::Runtime(ExplodingModel).load(store, Clarity::LogAgent(ExplodingModel).new(agent2, store: store), replay_llm_cache: true)
  ...
  it "raises ReplayDivergenceError on strict prompt-hash mismatch" do
    ...
    loaded = Clarity::Runtime(ExplodingModel).load(store, Clarity::LogAgent(ExplodingModel).new(agent, store: store), replay_strict: true)
    expect_raises(Clarity::ReplayDivergenceError) do
