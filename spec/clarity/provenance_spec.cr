...
describe Clarity::Provenance do
  it "creates provenance with required fields" do
    p = Clarity::Provenance.new(created_by: "user")
  ...
  it "GraphObject carries provenance after creation" do
    p = Clarity::Provenance.new(created_by: "user", caused_by_event: "evt_001")
    obj = Clarity::GraphObject.new("obj_001", "task", %({}), provenance: p)
  ...
  it "patch_object auto-applies without explicit proposal" do
    ...
    result.patch.status.should eq(Clarity::PatchState::Applied)
  ...
  it "Provenance carries through from event to object" do
    ...
    graph = Clarity::GraphProjection.empty.apply(event)
