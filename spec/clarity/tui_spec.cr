...
describe Clarity::TUI do
  it "creates an initial model in Input state" do
    model = Clarity::TUI::Model.new
    model.state.should eq(Clarity::TUI::State::Input)
  ...
  it "adds a user message on input" do
    ...
    model.state.should eq(Clarity::TUI::State::Processing)
  end
  ...
  it "adds a tool call message" do
    ...
    model = model.handle_tool_call("web_search", %({"q":"clarity"}))
    model.messages.size.should eq(2)
