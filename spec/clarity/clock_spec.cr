...
describe Clarity::Clock do
  it "WallClock returns the current UTC time" do
    clock = Clarity::WallClock.new
    t = clock.now
    t.should be_a(Time)
  ...
  it "FrozenClock always returns the same timestamp" do
    ...
    clock = Clarity::FrozenClock.new(frozen)
  ...
  it "FrozenClock defaults to a known timestamp" do
    clock = Clarity::FrozenClock.new
    clock.now.should eq(Clarity::FrozenClock::DEFAULT_TIME)
  end

  it "TickingClock advances monotonically" do
    start = Time.utc(2026, 7, 25, 12, 0, 0)
    clock = Clarity::TickingClock.new(start, step_seconds: 5)
  ...
  it "TickingClock defaults to 1-second steps" do
    start = Time.utc(2026, 7, 25, 12, 0, 0)
    clock = Clarity::TickingClock.new(start)
