...
module TraceSpecHelper
  ...
  def obj_event(seq : UInt64, id : String, type : String, data : String, caused_by : String?) : Clarity::Event
    Clarity::Event.new(
  ...
  def goal_event(seq : UInt64, id : String, goal : String) : Clarity::Event
    Clarity::Event.new(
  ...
  def chain_graph : Clarity::GraphProjection
    events = [
...
describe Clarity::Trace do
  ...
  it "reports missing objects" do
    g = TraceSpecHelper.chain_graph
    Clarity::Trace.causal_chain(g.events, g, "nope").should eq("(no such object: nope)")
  end
  ...
  it "detects cycles in the causal chain" do
    ...
    g = events.reduce(Clarity::GraphProjection.empty) { |proj, e| proj.apply(e) }
    # self-caused event
    cyclic = Clarity::Event.new(
    ...
    g2 = g.apply(cyclic)
    rendered = Clarity::Trace.causal_chain(g2.events, g2, "b#1")
