...
module Phase3SpecHelper
  ...
  def runtime(model = Phase3Model.new, tools : Array(Clarity::Tool) = [] of Clarity::Tool, budget : Clarity::Runtime::Budget = Clarity::Runtime::Budget.new) : Clarity::Runtime(Phase3Model)
    store = Clarity::MemoryEventStore.new
    agent = Crig::Agent(Phase3Model).new(model: model, preamble: "Use tools.")
    la = Clarity::LogAgent(Phase3Model).new(agent, store: store, max_turns: 2)
    Clarity::Runtime(Phase3Model).new(store: store, log_agent: la, tools: tools, budget: budget)
  end
end

describe Clarity::Runtime do
  it "runs a bounded quantum of loop steps" do
    runtime = Phase3SpecHelper.runtime(tools: [Clarity::Tool.new("search", "search") { |a| %({"results":[]}) }])
    runtime.run_quantum("Quantum me", steps: 2)
    runtime.response.should eq("")

    runtime2 = Phase3SpecHelper.runtime(tools: [Clarity::Tool.new("search", "search") { |a| %({"results":[]}) }])
  ...
  it "reports and resets the budget" do
    runtime = Phase3SpecHelper.runtime(budget: Clarity::Runtime::Budget.new(max_events: 10))
  ...
  it "tracks pending approvals and resolves them" do
    ...
    runtime.add_pending_approval(Clarity::ApprovalRequest.new("req_1", Clarity::ApprovalKind::Shell, "run command"))
