...
describe Clarity do
  describe ".parse_store_url" do
    ...
    it "parses a postgres URL and preserves the raw form" do
      u = Clarity.parse_store_url("postgres://u:p@host:5432/dbname")
    ...
    it "normalizes the postgresql alias to postgres" do
      u = Clarity.parse_store_url("postgresql://localhost/db")
      u.scheme.should eq("postgres")
    end

    it "rejects a bare path with a helpful message" do
      expect_raises(Clarity::InvalidStoreURL, /no scheme/) do
        Clarity.parse_store_url("run.db")
      end
      expect_raises(Clarity::InvalidStoreURL, /sqlite:\/\/\/run\.db/) do
        Clarity.parse_store_url("run.db")
      end
    end

    it "rejects an empty URL" do
      expect_raises(Clarity::InvalidStoreURL) do
        Clarity.parse_store_url("")
      end
    end

    it "rejects an unsupported scheme" do
      expect_raises(Clarity::InvalidStoreURL, /mysql/) do
        Clarity.parse_store_url("mysql://host/db")
      end
    end

    it "rejects a sqlite URL with no path" do
      expect_raises(Clarity::InvalidStoreURL) do
        Clarity.parse_store_url("sqlite://")
      end
    end

    it "rejects a postgres URL with no host or database" do
      expect_raises(Clarity::InvalidStoreURL) do
        Clarity.parse_store_url("postgres://")
      end
    end
  end
end
